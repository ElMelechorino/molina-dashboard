...
export function ScheduleWidget() {
  ...
  const getStatus = () => {
    if (currentClass) {
      return { label: 'En clase', color: 'bg-green-500', icon: '✅' };
    }
    const hour = currentTime.getHours();
    if (hour >= 7 && hour < 18) {
      return { label: 'Descanso', color: 'bg-yellow-500', icon: '🟡' };
    }
    return { label: 'Libre', color: 'bg-blue-500', icon: '🔵' };
  };
          ...
          <div className="flex w-full md:w-auto items-center justify-between md:justify-start gap-4">
            ...
            {activeSemester && (
              <button
                ...
                className="hidden lg:block px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                title="Ver horario"
              >
                📅 {activeSemester.name}
              </button>
                ...
                <div className="text-center sm:text-left">
                  ...
                  <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                    ...
                    {currentClass.modality === 'Presencial' && (
                      <span className="ml-2">📍 Salón {currentClass.room}</span>
                    )}
                  </div>
                </div>
            ...
            <div className="hidden md:block text-right">
              ...
              <div className="flex items-center justify-end gap-2 text-sm">
                ...
                {nextClass.class.modality === 'Virtual' ? (
                  <span className="text-purple-500 dark:text-purple-400">💻 Virtual</span>
                ) : (
                  <span className="text-green-500 dark:text-green-400">📍 {nextClass.class.room}</span>
                )}
              </div>
            </div>
          )}
