...
import {
  Video, BookOpen, NotebookText, CheckSquare, Calendar,
  PartyPopper, Sparkles, Clock, Zap, Bot, Search, Archive, Hand, CalendarDays
} from 'lucide-react';
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-3xl p-8 text-white relative overflow-hidden">
        ...
        <div className="relative">
          <h1 className="text-3xl font-bold mb-2 flex items-center gap-2">
            ¡Bienvenido{state.user?.email ? `, ${state.user.email.split('@')[0].charAt(0).toUpperCase() + state.user.email.split('@')[0].slice(1)}` : ''}! <Hand className="w-8 h-8" />
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-2">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
              <Calendar className="w-5 h-5 text-blue-500" /> Clases de hoy
            </h2>
            <button
              onClick={() => dispatch({ type: 'SET_VIEW', payload: 'schedule' })}
          ...
          {todayClasses.length === 0 ? (
            <EmptyState
              icon={<PartyPopper className="w-10 h-10 text-purple-500 mx-auto" />}
              title="¡No hay clases hoy!"
              description="Aprovecha para estudiar o descansar."
            />
          ) : upcomingTodayClasses.length === 0 ? (
            <EmptyState
              icon={<Sparkles className="w-10 h-10 text-yellow-500 mx-auto" />}
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
              <Clock className="w-5 h-5 text-orange-500" /> Tareas Próximas
            </h2>
          </div>
          ...
          {(() => {
            ...
            if (upcomingTasks.length === 0) {
              return (
                <EmptyState
                  icon={<PartyPopper className="w-10 h-10 text-green-500 mx-auto" />}
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
              <NotebookText className="w-5 h-5 text-green-500" /> Notas recientes
            </h2>
          </div>

          {recentNotes.length === 0 ? (
            <EmptyState
              icon={<NotebookText className="w-10 h-10 text-slate-400 mx-auto" />}
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-6">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2 mb-4">
            <Zap className="w-5 h-5 text-yellow-500" /> Acciones rápidas
          </h2>
